...
func (s *SubmitRayJobArgsBuilder) AddCommandFlags(command *cobra.Command) {
	...
	var (
		activeDeadline   time.Duration = time.Second * 120
		ttlAfterFinished time.Duration
		preStopCmd       string
	)

	command.Flags().StringVar(&s.args.Entrypoint, "entrypoint", "", "specify the command to be executed when submitting rayjob")
	_ = command.MarkFlagRequired("entrypoint")
	command.Flags().BoolVar(&s.args.ShutdownAfterJobFinishes, "shutdown-after-finished", false, "determine whether to delete the ray cluster once rayJob succeed or failed.")
	command.Flags().DurationVar(&ttlAfterFinished, "ttl-after-finished", ttlAfterFinished, "The TTL to clean up RayCluster. It's only working when ShutdownAfterJobFinishes set to true, like 60s.")
	...
	command.Flags().StringVar(&s.args.HeadGroupSpec.Cpu, "head-cpu", "1", "the cpu resource to HeadPod for the training, like 1 for 1 core.")
	command.Flags().StringVar(&s.args.HeadGroupSpec.Memory, "head-memory", "2Gi", "the memory resource to HeadPod for the training, like 2Gi.")
	...
	command.Flags().StringVar(&s.args.WorkerGroupSpec.Cpu, "worker-cpu", "1", "the cpu resource to WorkerPod for the training, like 1 for 1 core.")
	command.Flags().StringVar(&s.args.WorkerGroupSpec.Memory, "worker-memory", "1Gi", "the memory resource to WorkerPod for the training, like 1Gi.")
	command.Flags().IntVar(&s.args.WorkerGroupSpec.Gpu, "worker-gpu", 0, "the GPU count of each worker to run the training.")
	command.Flags().Int32Var(&s.args.WorkerGroupSpec.Replicas, "worker-replicas", 0, "worker-replicas is the number of desired Pods for this worker group, and the default value is 0.")
	command.Flags().Int32Var(&s.args.WorkerGroupSpec.MinReplicas, "worker-min-replicas", 0, "worker-min-replicas denotes the minimum number of desired Pods for this worker group, and the default value is 0.")
	command.Flags().Int32Var(&s.args.WorkerGroupSpec.MaxReplicas, "worker-max-replicas", 2147483647, "worker-max-replicas denotes the maximum number of desired Pods for this worker group.")
