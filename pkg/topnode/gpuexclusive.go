...
func NewGPUExclusiveNode(node *v1.Node, pods []*v1.Pod, index int, args buildNodeArgs) (Node, error) {
	return &gpuexclusive{
		node:       node,
		pods:       pods,
...
func (g *gpuexclusive) WideFormat() string {
	...
	lines = g.displayDeviceInfoUnderMetrics(lines, nodeInfo)
