...
func buildCommandArgs(t *task.Task) ([]interface{}, []interface{}) {
	shell := t.EffectiveShell()
	command := []interface{}{shell, "-c"}
	args := []interface{}{t.Run}
	return command, args
}
...
func buildRunPolicy(t *task.Task) (map[string]interface{}, error) {
	...
	if sp := buildSchedulingPolicy(t.Scheduling); sp != nil {
...
func buildSchedulingPolicy(s task.Scheduling) map[string]interface{} {
...
// buildMetadata creates a metadata map with name, labels, and annotations.
...
func buildContainer(name, image string, t *task.Task, roleEnvs map[string]task.EnvValue) map[string]interface{} {
	container := map[string]interface{}{
		"name":  name,
		"image": image,
	}
...
func buildRoleReplicaSpec(containerName string, t *task.Task, resources task.Resources, envs map[string]task.EnvValue, replicas int64, restartPolicy string, includeVolumes bool) (map[string]interface{}, error) {
	container := map[string]interface{}{
		"name":  containerName,
		"image": t.Image,
	}
