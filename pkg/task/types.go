...
// Validate checks that the Storage entry has exactly one storage type declared.
...
// Empty version defaults to DefaultSchemaVersion.
...
func Validate(t *Task) error {
	...
	if t.Lifecycle.CleanPodPolicy != "" {
		valid := map[string]bool{constants.CleanPodPolicyNone: true, constants.CleanPodPolicyRunning: true, constants.CleanPodPolicyAll: true}
		if !valid[t.Lifecycle.CleanPodPolicy] {
			return fmt.Errorf("invalid clean_pod_policy: %q (must be None, Running, or All)", t.Lifecycle.CleanPodPolicy)
		}
	}

	if t.Restart != "" {
		valid := map[string]bool{constants.RestartPolicyAlways: true, constants.RestartPolicyOnFailure: true, constants.RestartPolicyNever: true}
		if !valid[t.Restart] {
	...
	if t.ImagePullPolicy != "" {
		valid := map[string]bool{"Always": true, "IfNotPresent": true, "Never": true}
		if !valid[t.ImagePullPolicy] {
	...
	if t.Lifecycle.SuccessPolicy != "" {
		valid := map[string]bool{"ChiefWorker": true, "AllWorkers": true}
		if !valid[t.Lifecycle.SuccessPolicy] {
	...
	if t.Framework.Name == constants.FrameworkMPI {
		if t.Framework.Options.MPIImplementation != "" {
			valid := map[string]bool{"OpenMPI": true, "Intel": true, "MPICH": true}
			if !valid[t.Framework.Options.MPIImplementation] {
		...
		if t.Framework.Options.LauncherCreationPolicy != "" {
			valid := map[string]bool{"AtStartup": true, "WaitForWorkersReady": true}
			if !valid[t.Framework.Options.LauncherCreationPolicy] {
	...
	if a := t.Scheduling.Affinity; a != nil {
		...
		if a.Policy != "" && a.Policy != "none" {
			...
			valid := map[string]bool{"spread": true, "binpack": true, "none": true}
			if !valid[a.Policy] {
		...
		if a.Constraint != "" {
			valid := map[string]bool{"preferred": true, "required": true}
			if !valid[a.Constraint] {
...
// EffectiveShell returns the shell to use, defaulting to /bin/sh for
