...
func ServingJobExecCommand(args AcceptExecArgs) (error) {
	...
	clientset,err := util.CreateK8sClientSetWithConfig(args.Config)
	if err != nil {
		return err
	}
	...
	podName,err := job.GetTargetPod(args.Options.PodName)
	if err != nil {
	...
	if err := args.Options.Run(); err != nil {
		...
		errMsg := fmt.Sprintf("Error: exec in container failed,reason: %v,please use \"arena serve get\" to check it's status.",err)
