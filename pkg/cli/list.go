...
// extractPods reads pod-level information from the CRD status.
func extractPods(obj *unstructured.Unstructured) []client.PodInfo {
	...
	for _, val := range replicaStatuses {
		...
		// Read per-pod entries from labelSelector
		podList, podFound, _ := unstructured.NestedSlice(statusMap, "labelSelector")
		if podFound {
			for _, p := range podList {
				...
				name, _, _ := unstructured.NestedString(podMap, "name")
				podStatus, _, _ := unstructured.NestedString(podMap, "status")
				ip, _, _ := unstructured.NestedString(podMap, "ip")
				node, _, _ := unstructured.NestedString(podMap, "node")
