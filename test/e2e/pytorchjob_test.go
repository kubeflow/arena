...
var _ = Describe("PyTorch", func() {
	Context("Basic", func() {
		It("Should be able to manage pytorch job", func() {
			jobName := "pytorchjob-test"
			jobNamespace := "default"
			jobType := string(types.PytorchTrainingJob)

			var err error
			var output string

			By("Use arena to submit a pytorch job")
			...
			Expect(output).Should(ContainSubstring(fmt.Sprintf("pytorchjob.kubeflow.org/%s created", jobName)))
			out.Reset()

			By("Use arena to get the status of a pytorch job")
			getCmd := exec.Command(
				"arena",
				"get",
				fmt.Sprintf("--namespace=%s", jobNamespace),
			...
			By("Use arena to list all pytorch jobs")
			...
			By("Use arena to delete a pytorch job")
